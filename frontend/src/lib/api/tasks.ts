...
export function listTasks(projectKey: string, page = 1, pageSize = 20, filters: { statusId?: string; parentTaskId?: string; taskType?: string } = {}) { const params = new URLSearchParams({ page: String(page), page_size: String(pageSize) }); if (filters.statusId) params.set('status_id', filters.statusId); if (filters.parentTaskId) params.set('parent_task_id', filters.parentTaskId); if (filters.taskType) params.set('task_type', filters.taskType); return apiGet<TaskListResponse>(`/projects/${key(projectKey)}/tasks?${params}`); }
...
export function createTask(projectKey: string, input: { title: string; description?: string; priority?: string; task_type?: string; status_id?: string; assignee_id?: string | null; reviewer_id?: string | null; start_at?: string | null; due_at?: string | null }) { return apiPost<TaskView>(`/projects/${key(projectKey)}/tasks`, input); }
export function createSubtask(taskKey: string, input: { title: string; description?: string; priority?: string; task_type?: string; status_id?: string; assignee_id?: string | null; reviewer_id?: string | null; start_at?: string | null; due_at?: string | null }) { return apiPost<TaskView>(`/tasks/${key(taskKey)}/subtasks`, input); }
export function updateTask(taskKey: string, input: { title?: string; description?: string; priority?: string; task_type?: string; assignee_id?: string | null; reviewer_id?: string | null; start_at?: string | null; due_at?: string | null; parent_task_id?: string | null }) { return apiPatch<TaskView>(`/tasks/${key(taskKey)}`, input); }
