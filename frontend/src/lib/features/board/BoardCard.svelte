<script lang="ts">
  ...
  const isOverdue = $derived(Boolean(task.due_at && new Date(task.due_at) < new Date()));
  const isRoot = $derived(!task.parent_task_id);
  const visibleLabels = $derived(task.labels.slice(0, 3));
  const hiddenLabelCount = $derived(Math.max(0, task.labels.length - 3));
...
>
  ...
  {#if visibleLabels.length}
    <span class="card-labels">
      ...
      {/each}
      {#if hiddenLabelCount}<span class="label-more">+{hiddenLabelCount}</span>{/if}
    </span>
  {/if}
  <span class="card-bottom">
    <small class:danger={isOverdue}>{task.due_at ? `截止 ${new Date(task.due_at).toLocaleString('zh-CN', { month: 'numeric', day: 'numeric', hour: '2-digit', minute: '2-digit' })}` : isRoot && task.subtask_total ? '未设置截止时间' : ''}</small>
    <span class="card-bottom-right">
      {#if isRoot && task.subtask_total}
        <span class="subtask-progress" title="子任务完成进度">{task.subtask_done}/{task.subtask_total}</span>
      {/if}
...
<style>
  ...
  .board-card.overdue { border-left-color: var(--color-danger); }
  ...
  .label-more { align-self: center; color: var(--color-text-muted); font-size: 11px; }
  ...
  .board-card small.danger { color: var(--color-danger); font-weight: 500; }
  .card-bottom-right { display: inline-flex; align-items: center; gap: 8px; min-width: 0; }
  .subtask-progress { padding: 1px 6px; border-radius: 999px; background: var(--color-hover); color: var(--color-text-muted); font-family: var(--font-mono); font-size: 11px; }
