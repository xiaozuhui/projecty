...
mod tests {
    ...
    #[test]
    fn root_task_is_allowed() {
        assert_eq!(
            validate_two_level_task_rule(&NewTaskParent {
                parent_task_id: None,
    ...
    #[test]
    fn subtask_cannot_have_subtask() {
        assert_eq!(
            validate_two_level_task_rule(&NewTaskParent {
                parent_task_id: Some(Uuid::nil()),
                parent_already_has_parent: true
            }),
            Err(TaskRuleError::SubtaskCannotHaveSubtask)
        );
    }
}
